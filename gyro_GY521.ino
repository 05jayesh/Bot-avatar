...
void recordAccelRegisters() {
  ...
  accelY = Wire.read()<<8|Wire.read(); //Store first two bytes into accelX
  accelX = Wire.read()<<8|Wire.read(); //Store middle two bytes into accelY
  accelZ = Wire.read()<<8|Wire.read(); //Store last two bytes into accelZ
  processAccelData();
}

void processAccelData(){
  gForceX = (accelX / 16384.0)*90;
  gForceY = (accelY / 16384.0)*90; 
  gForceZ = (accelZ / 16384.0)*90;
